...
for n = 1:iteration
    for k = 2:45
        ...
        SAR(1,k)=((ex(1,k)^2)*s(1,k))/(2*(1100));
    end
    source = sin(2*pi*900*10^(6)*dt(1,1)*n);       
     ex(1,2) = source;
    ...
    plot(SAR);grid
    grid on
    axis([0 m -0.0005 0.0005]); 
    line([k1,k1],[-.0005,.0005],'Color','r');
